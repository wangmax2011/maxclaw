...
scheduleCommand
  ...
  .action(async (projectName, scheduleName, options) => {
    try {
      ...
      // Validate task type
      const validTypes = ['reminder', 'backup', 'command', 'skill', 'github-sync'];
      ...
      // Check if project is linked to GitHub for github-sync type
      if (options.type === 'github-sync') {
        const { getProjectGitHubConfig } = await import('./github-sync.js');
        const config = getProjectGitHubConfig(project.id);
        if (!config.repo) {
          console.error(`❌ Project "${projectName}" is not linked to a GitHub repository`);
          console.error(`   Run: maxclaw github link ${projectName} <owner/repo>`);
          process.exit(1);
        }
      }
...
const notionCommand = program.command('notion').description('Manage Notion integration');

// notion link
notionCommand
  .command('link <project> <page-id>')
  .description('Link a project to a Notion page')
  .action(async (projectName, pageId) => {
    try {
      const { linkProjectToNotion } = await import('./notion-sync.js');
      const project = findProjectByName(projectName);
      if (!project) {
        console.error(`❌ Project not found: ${projectName}`);
        process.exit(1);
      ...
      await linkProjectToNotion(project.id, pageId);
      console.log(`✅ Linked project "${project.name}" to Notion page: ${pageId}`);
    } catch (error) {
      console.error(`❌ Error: ${error}`);
      process.exit(1);
    }
...
notionCommand
  ...
  .description('Unlink a project from Notion')
  .action(async (projectName) => {
    try {
      const { unlinkProjectFromNotion } = await import('./notion-sync.js');
      const project = findProjectByName(projectName);
      if (!project) {
        console.error(`❌ Project not found: ${projectName}`);
        process.exit(1);
      ...
      await unlinkProjectFromNotion(project.id);
      console.log(`✅ Unlinked project "${project.name}" from Notion`);
    } catch (error) {
      console.error(`❌ Error: ${error}`);
      process.exit(1);
    }
...
notionCommand
  ...
  .description('Manually sync project to Notion')
  .option('-a, --all-sessions', 'Sync all session summaries')
  .action(async (projectName, options) => {
    try {
      const { syncProjectToNotion, syncAllSessionsToNotion } = await import('./notion-sync.js');
      const project = findProjectByName(projectName);
      if (!project) {
        console.error(`❌ Project not found: ${projectName}`);
        process.exit(1);
      ...
      console.log(`🔄 Syncing project "${project.name}" to Notion...`);

      await syncProjectToNotion(project.id);
      console.log('✅ Project metadata synced');

      if (options.allSessions) {
        const result = await syncAllSessionsToNotion(project.id);
        console.log(`✅ Synced ${result.synced} session summaries (${result.failed} failed)`);
      }
    } catch (error) {
      console.error(`❌ Error: ${error}`);
      process.exit(1);
...
// notion create-page
notionCommand
  .command('create-page <project>')
  .description('Create a new Notion page for a project')
  .option('-p, --parent <page-id>', 'Parent Notion page ID (or set NOTION_PARENT_PAGE_ID)')
  .action(async (projectName, options) => {
    try {
      const { createProjectPage } = await import('./notion-sync.js');
      const project = findProjectByName(projectName);
      if (!project) {
        console.error(`❌ Project not found: ${projectName}`);
        process.exit(1);
      ...
      const parentId = options.parent || process.env.NOTION_PARENT_PAGE_ID;
      if (!parentId) {
        console.error('❌ Parent page ID required. Provide --parent or set NOTION_PARENT_PAGE_ID');
        process.exit(1);
      }

      console.log(`📄 Creating Notion page for "${project.name}"...`);
      const pageId = await createProjectPage(project.id, parentId);
      if (pageId) {
...
notionCommand
  ...
  .action(async () => {
    try {
      const { isNotionConfigured } = await import('./notion-client.js');
      ...
      if (!configured) {
        ...
        console.log('  2. Set the NOTION_TOKEN environment variable');
        console.log('  3. Share your Notion pages with the integration');
      } else {
        const projects = listProjects();
        const linkedProjects = projects.filter((p) => p.notionPageId);
        console.log(`  Linked projects: ${linkedProjects.length}/${projects.length}`);

        if (linkedProjects.length > 0) {
          console.log('\n  Linked projects:');
          for (const project of linkedProjects) {
            console.log(`    • ${project.name} → ${project.notionPageId}`);
          }
        }
      }
      console.log();
...
githubCmd
  ...
  .action(async (projectName, repo, options) => {
    try {
      ...
      const { linkProjectToGitHub } = await import('./github-sync.js');
      const result = await linkProjectToGitHub(project.id, repo, options.token);

      if (result.success) {
        console.log(`✅ Linked project "${project.name}" to GitHub repository: ${repo}`);
      } else {
...
githubCmd
  ...
  .action(async (projectName) => {
    try {
      ...
      const { unlinkProjectFromGitHub, getProjectGitHubConfig } = await import('./github-sync.js');
      const config = getProjectGitHubConfig(project.id);

      if (!config.repo) {
        console.log(`ℹ️ Project "${project.name}" is not linked to a GitHub repository`);
        return;
      }

      unlinkProjectFromGitHub(project.id);
      console.log(`✅ Unlinked project "${project.name}" from GitHub repository: ${config.repo}`);
    } catch (error) {
      console.error(`❌ Error: ${error}`);
      process.exit(1);
    }
...
githubCmd
  .command('sync [project]')
  .description('Sync GitHub issues, PRs, and commits for a project (or all projects)')
  .option('-i, --issues', 'Sync only issues')
  .option('-p, --prs', 'Sync only pull requests')
  .option('-c, --commits', 'Sync only commits')
  .action(async (projectName, options) => {
    try {
      const { syncAll, syncIssues, syncPullRequests, syncCommits, getProjectGitHubConfig } = await import('./github-sync.js');

      const projectsToSync = projectName
        ? [findProjectByName(projectName)].filter(Boolean)
        : getAllProjects();

      if (projectsToSync.length === 0) {
        console.error(projectName ? `❌ Project not found: ${projectName}` : '❌ No projects found');
        process.exit(1);
      }

      const syncTypes = [];
      if (options.issues) syncTypes.push('issues');
      if (options.prs) syncTypes.push('PRs');
      if (options.commits) syncTypes.push('commits');
      if (syncTypes.length === 0) syncTypes.push('all');
      ...
      for (const project of projectsToSync) {
        ...
        const config = getProjectGitHubConfig(project.id);
        if (!config.repo) {
          console.log(`⏭️ Skipping "${project.name}" - not linked to GitHub`);
          continue;
        }

        console.log(`\n🔄 Syncing ${syncTypes.join(', ')} for "${project.name}" (${config.repo})...`);

        let result;
        if (options.issues) {
          result = await syncIssues(project.id);
        } else if (options.prs) {
          result = await syncPullRequests(project.id);
        } else if (options.commits) {
          result = await syncCommits(project.id);
        } else {
          result = (await syncAll(project.id)).results.issues;
        }

        if (result.success) {
          console.log(`✅ Sync completed for "${project.name}"`);
          if (result.issues) {
            console.log(`   Issues: ${result.issues.total} total (${result.issues.new} new, ${result.issues.updated} updated)`);
          }
          if (result.pullRequests) {
            console.log(`   PRs: ${result.pullRequests.total} total (${result.pullRequests.new} new, ${result.pullRequests.updated} updated)`);
          }
          if (result.commits) {
            console.log(`   Commits: ${result.commits.total} total (${result.commits.new} new)`);
          }
        } else {
          console.error(`❌ Sync failed for "${project.name}": ${result.error}`);
        }
      }
    } catch (error) {
      console.error(`❌ Error: ${error}`);
...
githubCmd
  ...
  .description('View cached GitHub issues')
  .option('-s, --state <state>', 'Filter by state (open/closed/all)', 'open')
  .option('-l, --limit <limit>', 'Maximum number of issues', '20')
  .action(async (projectName, options) => {
    try {
      const { getCachedIssues, getProjectGitHubConfig } = await import('./github-sync.js');
      ...
      for (const project of projectsToShow) {
        ...
        const config = getProjectGitHubConfig(project.id);
        if (!config.repo) {
          console.log(`\n⏭️ "${project.name}" - not linked to GitHub`);
          continue;
        }
        ...
        console.log(`\n📋 GitHub Issues for "${project.name}" (${config.repo}):`);

        if (issues.length === 0) {
          console.log('   No issues found. Run `maxclaw github sync` to fetch issues.');
          continue;
        }
        ...
        for (const issue of issues) {
          ...
          console.log(`   ${state} #${issue.issueNumber}: ${issue.title}${labels}`);
...
githubCmd
  .command('prs [project]')
  .description('View cached GitHub pull requests')
  .option('-s, --state <state>', 'Filter by state (open/closed/all)', 'open')
  .option('-l, --limit <limit>', 'Maximum number of PRs', '20')
  .action(async (projectName, options) => {
    try {
      const { getCachedPullRequests, getProjectGitHubConfig } = await import('./github-sync.js');
      ...
      for (const project of projectsToShow) {
        ...
        const prs = getCachedPullRequests(project.id, {
          state: options.state as 'open' | 'closed' | 'all',
          limit: parseInt(options.limit, 10),
        });

        console.log(`\n🔀 GitHub Pull Requests for "${project.name}" (${config.repo}):`);

        if (prs.length === 0) {
          console.log('   No pull requests found. Run `maxclaw github sync` to fetch PRs.');
          continue;
        }

        for (const pr of prs) {
          const state = pr.state === 'open' ? '🟢' : pr.merged ? '🟣' : '🔴';
          const draft = pr.draft ? ' [DRAFT]' : '';
          const labels = pr.labels.length > 0 ? ` [${pr.labels.join(', ')}]` : '';
          console.log(`   ${state} #${pr.prNumber}: ${pr.title}${draft}${labels}`);
          console.log(`      Branch: ${pr.branch} → ${pr.baseBranch}`);
          console.log(`      ${pr.url}`);
        }
      }
    } catch (error) {
...
githubCmd
  ...
  .action(async (projectName, title, options) => {
    try {
      ...
      const { createGitHubIssue } = await import('./github-sync.js');
      const result = await createGitHubIssue(project.id, {
        title,
        body: options.body,
      ...
      if (result.success && result.issue) {
        console.log(`✅ Created GitHub issue #${result.issue.number}: ${result.issue.title}`);
...
githubCmd
  ...
  .action(async (projectName) => {
    try {
      ...
      const { getRepositoryInfo, getProjectGitHubConfig, getCachedIssues, getCachedPullRequests } = await import('./github-sync.js');
      const config = getProjectGitHubConfig(project.id);

      if (!config.repo) {
        console.log(`ℹ️ Project "${project.name}" is not linked to a GitHub repository`);
        console.log(`\n💡 Run "maxclaw github link ${projectName} <owner/repo>" to link.`);
        return;
      }
      ...
      const repoInfo = await getRepositoryInfo(project.id);
      ...
      // Show cached stats
      const issues = getCachedIssues(project.id, { state: 'all', limit: 1000 });
      const prs = getCachedPullRequests(project.id, { state: 'all', limit: 1000 });

      console.log(`\n📊 Local Cache:`);
      console.log(`   Issues: ${issues.length} cached`);
      console.log(`   Pull Requests: ${prs.length} cached`);
    } catch (error) {
      console.error(`❌ Error: ${error}`);
      process.exit(1);
    }
